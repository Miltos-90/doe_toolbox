...
def fullfact(levels: list) -> np.array:
    ...
    outProd = list( product(*arrays) ) # Compute outer product
    outProd = np.array(outProd)
    outProd = np.fliplr(outProd)       # Convert to Yates standard ordering: 1st factor 
    # changes levels with each run, 2nd every other run, 3rd every 4th run, and so forth.
